...
AudioMixer4              mixer1;         //xy=253,137
AudioEffectDelay         delay1;         //xy=504,361
AudioMixer4              mixer2;         //xy=566,152
...
void loop() {
  ...
  int mixKnob = analogRead(A7);
  float mix = 1.0-(float)mixKnob / 1050.0;
  mixer2.gain(0, mix);

  
  /*
  int mix = analogRead(A7);  // knob = 0 to 1023
  mix = map(mix, 0, 1023, 0, 10);
  float gain1 = ((float)mix/10) ;// 1023.0;
  //float gain1 = (float)mix / 1023.0;
