...
Atm_encoder& Atm_encoder::begin( int pin1, int pin2, int divider /* = 1 */ ) {
  ...
  const static state_t state_table[] PROGMEM = {
    ...
    /* IDLE  */       -1,    LP_IDLE,    -1,     	-1,		WAIT_UP,  WAIT_DOWN,   -1,
	/* WAIT_UP */     -1,    	-1,      -1,    	UP,			-1,     -1,   		-1,
	/* WAIT_DOWN*/     -1,    	-1,      -1,    	DOWN,		-1,     -1,   		-1,   
...
int Atm_encoder::event( int id ) {
  switch ( id ) {
    case EVT_UP:
      return enc_direction == +1 && ( enc_counter % divider == 0 ) && m_debounce_up > DEBOUNCE;
    case EVT_DOWN:
      return enc_direction == -1 && ( enc_counter % divider == 0 ) && m_debounce_down > DEBOUNCE;
    case EVT_TIMER:
      ...
      if ( enc_direction != 0 ) {
        ...
        if ( ( enc_counter != 0 ) && ( enc_counter % divider == 0 ) ) {
          if ( !count( enc_direction ) ) {
			  //attempt - if debounce timer is not expired, disregard
			  if( timer_debounce )
            enc_direction = 0;
          }
        }
